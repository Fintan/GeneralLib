...
var GeneralLibTests = function() {
	this.runner = new utest.Runner();
	this.runner.addCase(new tests.TestEvents());
	utest.ui.Report.create(this.runner);
	this.runner.run();
};
$hxClasses["GeneralLibTests"] = GeneralLibTests;
GeneralLibTests.__name__ = ["GeneralLibTests"];
GeneralLibTests.main = function() {
	new GeneralLibTests();
}
GeneralLibTests.prototype = {
	runner: null
	,__class__: GeneralLibTests
}
var Hash = function() {
	this.h = { };
};
...
var fboyle = {}
fboyle.bbone = {}
fboyle.bbone.Events = function() {
	this.eventSplitter = " ";
	this._callbacks = new Hash();
	this.bind = $bind(this,this.on);
	this.unbind = $bind(this,this.off);
};
$hxClasses["fboyle.bbone.Events"] = fboyle.bbone.Events;
fboyle.bbone.Events.__name__ = ["fboyle","bbone","Events"];
fboyle.bbone.Events.prototype = {
	trigger: function(events_,rest) {
		var event, calls, list = null, i, length, args, all = null;
		if(rest == null) rest = [];
		calls = this._callbacks;
		if(calls == null) return this;
		var events3 = events_.split(this.eventSplitter);
		while((event = events3.shift()) != null) {
			all = calls.exists("all")?calls.get("all"):null;
			if(all != null) all = all.slice(0);
			list = calls.exists(event)?calls.get(event):null;
			if(list != null) list = list.slice(0);
			if(list != null) {
				length = list.length;
				i = 0;
				while(i < length) {
	...
	,off: function(events_,callback_,context) {
		var event, calls, list, i;
		calls = this._callbacks;
		if(this._callbacks == null || Lambda.empty(this._callbacks)) return this;
		...
		var events2 = events_ != null?events_.split(this.eventSplitter):keys;
		while((event = events2.shift()) != null) {
			...
			if(list != null || !(callback_ != null || context != null)) {
				if(callback_ != null) {
					...
					while(_g1 < _g) {
						var i1 = _g1++;
						if(list[i1] == callback_) {
							HxOverrides.remove(list,list[i1]);
							if(i1 < list.length) HxOverrides.remove(list,list[i1]);
						}
					}
					calls.set(event,list);
				} else calls.remove(event);
				continue;
			}
			i = list.length - 2;
			while(i >= 0) {
				if(!(callback_ != null && list[i] != callback_ || context != null && list[i + 1] != context)) list.splice(i,2);
				i -= 2;
			}
		}
		return this;
	}
	,on: function(events_,callback_,context) {
		var calls, event, list;
		if(callback_ == null) return this;
		var events = events_.split(this.eventSplitter);
		calls = this._callbacks != null?this._callbacks:new Hash();
		while((event = events.shift()) != null) {
			if(calls.exists(event)) list = calls.get(event); else {
				list = [];
				calls.set(event,list);
			}
			list.push(callback_);
...
$hxClasses["tests.TestEvents"] = tests.TestEvents;
tests.TestEvents.__name__ = ["tests","TestEvents"];
tests.TestEvents.prototype = {
	testRemoveAllEventsForASpecificCallback: function() {
		var obj = { };
		var e = new fboyle.bbone.Events();
		var success = function() {
			utest.Assert.isTrue(true,null,{ fileName : "TestEvents.hx", lineNumber : 258, className : "tests.TestEvents", methodName : "testRemoveAllEventsForASpecificCallback"});
		};
		var fail = function() {
			utest.Assert.isTrue(false,null,{ fileName : "TestEvents.hx", lineNumber : 260, className : "tests.TestEvents", methodName : "testRemoveAllEventsForASpecificCallback"});
		};
	...
	,testAllCallbackListIsRetrievedAfterEachEvent: function() {
		var counter = 0;
		var obj = { };
		var e = new fboyle.bbone.Events();
		var incr = function() {
		...
		e.trigger("x y");
		utest.Assert.equals(counter,2,null,{ fileName : "TestEvents.hx", lineNumber : 250, className : "tests.TestEvents", methodName : "testAllCallbackListIsRetrievedAfterEachEvent"});
	}
	,testCallbackListIsNotAlteredDuringTrigger: function() {
		var counter = 0;
		var obj = { };
		var e = new fboyle.bbone.Events();
		var incr = function() {
		...
		e.trigger("event");
		utest.Assert.equals(counter,0,"bind does not alter callback list",{ fileName : "TestEvents.hx", lineNumber : 219, className : "tests.TestEvents", methodName : "testCallbackListIsNotAlteredDuringTrigger"});
		e.off();
		e.on("event",function() {
			e.off("event",incr);
			e.off("all",incr);
		...
		e.on("all",incr);
		e.trigger("event");
		utest.Assert.equals(counter,2,"unbind does not alter callback list",{ fileName : "TestEvents.hx", lineNumber : 234, className : "tests.TestEvents", methodName : "testCallbackListIsNotAlteredDuringTrigger"});
	}
	,testNestedTriggerWithUnbind: function() {
		var _g = this;
		var obj = { counter : 0};
		var e = new fboyle.bbone.Events();
		this.incr1 = function() {
			obj.counter = Reflect.getProperty(obj,"counter") + 1;
			e.off("event",_g.incr1);
			e.trigger("event");
		};
		this.incr2 = function() {
			obj.counter = Reflect.getProperty(obj,"counter") + 1;
		};
		e.on("event",this.incr1).on("event",this.incr2).trigger("event");
		utest.Assert.equals(obj.counter,3,"counter should have been incremented three times",{ fileName : "TestEvents.hx", lineNumber : 203, className : "tests.TestEvents", methodName : "testNestedTriggerWithUnbind"});
	}
	,incr2: null
	,incr1: null
	,testBindACallbackWithASuppliedContext: function() {
		var _g = this;
		this.assertTrue = function() {
			utest.Assert.isTrue(true,"`this` was bound to the callback",{ fileName : "TestEvents.hx", lineNumber : 174, className : "tests.TestEvents", methodName : "testBindACallbackWithASuppliedContext"});
		};
		var e = new fboyle.bbone.Events();
		e.on("event",function() {
			_g.assertTrue();
	...
	,assertTrue: null
	,testTwoBindsThatUnbindThemselves: function() {
		var _g = this;
		var obj = { counterA : 0, counterB : 0};
		var e = new fboyle.bbone.Events();
		this.incrA = function() {
			obj.counterA = Reflect.getProperty(obj,"counterA") + 1;
			e.off("event",_g.incrA);
		};
		this.incrB = function() {
			obj.counterB = Reflect.getProperty(obj,"counterB") + 1;
			e.off("event",_g.incrB);
		};
		e.on("event",this.incrA);
		e.on("event",this.incrB);
		e.trigger("event");
		e.trigger("event");
		e.trigger("event");
		utest.Assert.equals(obj.counterA,1,"counterA should have only been incremented once.",{ fileName : "TestEvents.hx", lineNumber : 164, className : "tests.TestEvents", methodName : "testTwoBindsThatUnbindThemselves"});
		utest.Assert.equals(obj.counterB,1,"counterB should have only been incremented once.",{ fileName : "TestEvents.hx", lineNumber : 165, className : "tests.TestEvents", methodName : "testTwoBindsThatUnbindThemselves"});
	}
	,incrB: null
	,incrA: null
	,testUnbindACallbackInTheMidstOfItFiring: function() {
		var _g = this;
		var obj = { counter : 0};
		var e = new fboyle.bbone.Events();
		this.callback_ = function() {
			obj.counter = Reflect.getProperty(obj,"counter") + 1;
			e.off("event",_g.callback_);
		};
		e.on("event",this.callback_);
		e.trigger("event");
		e.trigger("event");
		e.trigger("event");
		utest.Assert.equals(obj.counter,1,"the callback should have been unbound.",{ fileName : "TestEvents.hx", lineNumber : 138, className : "tests.TestEvents", methodName : "testUnbindACallbackInTheMidstOfItFiring"});
	}
	,callback_: null
	,testBindTwoCallbacksUnbindOnlyOne: function() {
		var obj = { counterA : 0, counterB : 0};
		var e = new fboyle.bbone.Events();
		...
		e.trigger("event");
		utest.Assert.equals(obj.counterA,1,"counterA should have only been incremented once.",{ fileName : "TestEvents.hx", lineNumber : 113, className : "tests.TestEvents", methodName : "testBindTwoCallbacksUnbindOnlyOne"});
		utest.Assert.equals(obj.counterB,2,"counterB should have been incremented twice.",{ fileName : "TestEvents.hx", lineNumber : 114, className : "tests.TestEvents", methodName : "testBindTwoCallbacksUnbindOnlyOne"});
	}
	,testOnThenUnbindAllFunctions: function() {
		var obj = { counter : 0};
		var e = new fboyle.bbone.Events();
		e.on("event",function() {
			obj.counter += 1;
		...
		e.off("event");
		e.trigger("event");
		utest.Assert.equals(obj.counter,1,"counter should have only been incremented once.",{ fileName : "TestEvents.hx", lineNumber : 96, className : "tests.TestEvents", methodName : "testOnThenUnbindAllFunctions"});
	}
	,testTriggerAllForEachEvent: function() {
		...
		e.trigger("a b");
		utest.Assert.equals(obj.counter,2,null,{ fileName : "TestEvents.hx", lineNumber : 83, className : "tests.TestEvents", methodName : "testTriggerAllForEachEvent"});
	}
	,testBindingAndTriggeringMultipleEvents: function() {
		var obj = { counter : 0};
		var e = new fboyle.bbone.Events();
		e.on("a b c",function() {
			obj.counter += 1;
		...
		e.trigger("a");
		utest.Assert.equals(obj.counter,1,null,{ fileName : "TestEvents.hx", lineNumber : 54, className : "tests.TestEvents", methodName : "testBindingAndTriggeringMultipleEvents"});
		e.trigger("a b");
		utest.Assert.equals(obj.counter,3,null,{ fileName : "TestEvents.hx", lineNumber : 57, className : "tests.TestEvents", methodName : "testBindingAndTriggeringMultipleEvents"});
		e.trigger("c");
		utest.Assert.equals(obj.counter,4,null,{ fileName : "TestEvents.hx", lineNumber : 60, className : "tests.TestEvents", methodName : "testBindingAndTriggeringMultipleEvents"});
		e.off("a c");
		e.trigger("a b c");
		utest.Assert.equals(obj.counter,5,null,{ fileName : "TestEvents.hx", lineNumber : 64, className : "tests.TestEvents", methodName : "testBindingAndTriggeringMultipleEvents"});
	}
	,testOnandTrigger: function() {
		var obj = { counter : 0};
		var e = new fboyle.bbone.Events();
		e.on("event",function() {
			obj.counter += 1;
		...
		e.trigger("event");
		utest.Assert.equals(obj.counter,1,"counter should be incremented.",{ fileName : "TestEvents.hx", lineNumber : 37, className : "tests.TestEvents", methodName : "testOnandTrigger"});
		e.trigger("event");
		e.trigger("event");
		e.trigger("event");
		e.trigger("event");
		utest.Assert.equals(obj.counter,5,"counter should be incremented five times.",{ fileName : "TestEvents.hx", lineNumber : 42, className : "tests.TestEvents", methodName : "testOnandTrigger"});
	}
	,testOptionalArgumentsPassing: function() {
		var e = new fboyle.bbone.Events();
		e.on("all",function(event,arg1,arg2) {
			utest.Assert.equals(arg1,"hello","second arg on \"all\" handler should be the first custom value.",{ fileName : "TestEvents.hx", lineNumber : 16, className : "tests.TestEvents", methodName : "testOptionalArgumentsPassing"});
			utest.Assert.equals(arg2,"fintan","third arg on \"all\" handler should be the second custom value.",{ fileName : "TestEvents.hx", lineNumber : 17, className : "tests.TestEvents", methodName : "testOptionalArgumentsPassing"});
		});
		e.on("alert",function(arg1,arg2) {
			utest.Assert.equals(arg1,"hello","first arg on \"all\" handler should be the first custom value.",{ fileName : "TestEvents.hx", lineNumber : 21, className : "tests.TestEvents", methodName : "testOptionalArgumentsPassing"});
			utest.Assert.equals(arg2,"fintan","second arg on \"all\" handler should be the second custom value.",{ fileName : "TestEvents.hx", lineNumber : 22, className : "tests.TestEvents", methodName : "testOptionalArgumentsPassing"});
		});
		e.trigger("alert",["hello","fintan"]);
	}
	,__class__: tests.TestEvents
}
var utest = {}
